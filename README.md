...
以下编解码器和功能已通过独立测试程序严格验证（43 项通过）：

**编解码器可用性**（24 项）

| 类别 | 已验证项 |
|------|----------|
| 视频编码 | libx264, libx265, h264_amf, hevc_amf, libsvtav1, libvpx-vp9 |
| 音频编码 | aac（含 round-trip 验证） |
| 视频解码 | h264, hevc, vp9, av1 |
| 音频解码 | aac, mp3 |
| 图片编解码 | png, mjpeg (JPEG), bmp, tiff |

**质量评估**（6 项，含严格准确性验证）

| 指标 | 验证方式 | 结论 |
|------|----------|------|
| PSNR | 相同帧 → inf dB | 完全正确 |
| SSIM | 相同帧 → 1.000000 | 完全正确 |
| VMAF | 相同帧 → 97.42（模型天花板） | 完全正确 |

**音频编解码**（4 项）

AAC 编码 → 解码 round-trip，跨相关峰值为 0.999995，解码信号与原始信号一致。

**图片格式转换**（4 项）

PNG → JPEG / BMP / TIFF，均通过内容校验（像素非纯色，有实际变化）。

**SVG 光栅化**（5 项）

多图形组合渲染、不等比缩放宽高比保持、32×32 图标、无效输入降级、缺失文件处理。

> 测试程序位于 `tests/`，每种功能独立测试，每轮产生带时间戳的隔离输出目录。
> 运行全部测试：`mingw32-make test`
